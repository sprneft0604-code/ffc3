...
# Minimal compatibility helpers (placeholders for optional features)
...
def _instantiate_activation(activation_ctor):
    # Try to instantiate with inplace, otherwise without args
    try:
        return activation_ctor(inplace=True)
    except Exception:
        try:
...
class FFC(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, ratio_gin, ratio_gout, stride=1, padding=0,
                 dilation=1, groups=1, bias=False, enable_lfu=True, padding_type='reflect', gated=False, **spectral_kwargs):
...
class FFC_BN_ACT(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, ratio_gin, ratio_gout, stride=1, padding=0, dilation=1, groups=1, bias=False, norm_layer=nn.BatchNorm2d, activation_layer=nn.Identity, padding_type='reflect', enable_lfu=True, **kwargs):
        ...
        # robust activation instantiation
        try:
            self.act_l = activation_layer(inplace=True)
        except Exception:
            try:
                self.act_l = activation_layer()
            except Exception:
...
class FFCResnetBlock(nn.Module):
    def __init__(self, dim, padding_type='reflect', norm_layer=nn.BatchNorm2d, activation_layer=nn.ReLU, dilation=1, spatial_transform_kwargs=None, inline=False, residual=True, **conv_kwargs):
        super().__init__()
